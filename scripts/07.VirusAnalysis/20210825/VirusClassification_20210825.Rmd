...
```{r}
files <- list.files("/mnt/raid61/Personal_data/songjunwei/DRS_RTA/batch2_fq/20210825_1639_MN35929_FAQ89985_6914f112", "fast5$", recursive = TRUE, full.names = TRUE)
```
...
saveRDS(barcodeSigsBinMat, "./analysis/07.VirusClassification/01.NormalBarcodeSignal/20210825/barcodeSigsBinMat.Rds")
```
...
saveRDS(ROC_Tab, "./analysis/07.VirusClassification/02.Prediction/20210825/BarcodesPrediction.Rds")
```

# Alignment

```{r}
bams <- list.files("/mnt/raid61/Personal_data/songjunwei/DRS_RTA/align/batch2", "bam", full.names = TRUE)
```
...
saveRDS(aligns, "./analysis/07.VirusClassification/02.Prediction/20210825/AlignmentResult.Rds")
```
...
```{r}
parasite_aligns <- aligns[Species %in% c("PbergheiANKA", "PRRSV", "SARS_Cov_2", "SVV")]
parasite_result <- merge(parasite_aligns, Preds, by = "qname")
parasite_result[, Prediction := plyr::mapvalues(pred, 
                                                from = c("RTA-37", "RTA-33", "RTA-10", "RTA-27", "RTA-08"), 
                                                to = c("SARS_Cov_2", "SVV", "PRRSV", "PbergheiANKA", "PbergheiANKA"))]
parasite_result$PP <- apply(parasite_result[, grepl("RTA", colnames(parasite_result)), with = F], 1, max)
```

```{r}
Mat_parasite <- parasite_result[, .(qname, Species, Prediction, PP)]
Mat_parasite[, Species := factor(Species, levels = levels(Mat_parasite$Prediction))]

postResample(pred = Mat_parasite$Prediction, obs = Mat_parasite$Species)
confusionMatrix(data = Mat_parasite$Prediction, reference = Mat_parasite$Species)
```
...
```{r}
host_aligns <- aligns[Species %in% c("HomSap", "MusMus", "SusScr")]
host_result <- merge(host_aligns, Preds, by = "qname")
host_result[, Prediction := plyr::mapvalues(pred, 
                                            from = c("RTA-37", "RTA-33", "RTA-10", "RTA-27", "RTA-08"), 
                                            to = c("HomSap", "SusScr", "SusScr", "MusMus", "MusMus"))]
host_result$PP <- apply(host_result[, grepl("RTA", colnames(host_result)), with = F], 1, max)
```

```{r}
Mat_host <- host_result[, .(qname, Species, Prediction, PP)]
Mat_host[, Species := factor(Species, levels = levels(Mat_host$Prediction))]

postResample(pred = Mat_host$Prediction, obs = Mat_host$Species)
confusionMatrix(data = Mat_host$Prediction, reference = Mat_host$Species)
```


```{r}
Preds <- merge(aligns, Preds, by = "qname")
Preds$pred <- plyr::mapvalues(Preds$pred,
                              from = c("RTA-37", "RTA-33", "RTA-10", "RTA-27", "RTA-08"),
                              to = c("SARS_Cov_2", "SVV", "PRRSV", "PbergheiANKA", "PbergheiANKA"))
Preds$PP <- apply(Preds[, grepl("RTA", colnames(Preds)), with = F], 1, max)
```
...
```{r}
Mat <- Preds[, .(qname, Species, pred, PP)]
Mat$pred <- factor(Mat$pred)
Mat$Species <- factor(Mat$Species, levels = levels(Mat$pred))
```

```{r}
postResample(pred = Mat$pred, obs = Mat$Species)
```

```{r}
confusionMatrix(data = Mat$pred, reference = Mat$Species)
```
